...
![App Screenshot](https://c2.staticflickr.com/8/7857/45926354015_c7ec87b7cf_h.jpg)
...
먼저 웹뷰의 페이지에서는 기본적으로 상단의 상태 막대를 안전 영역으로 처리한다. 그래서 고정 영역을 예로 들어 `top: 0`로 설정해도 상태 막대 바로 아래에 자리를 잡는다. 화면 전체를 웹 영역에 넣고 처리하고 싶은 경우에는 `viewport-fit`을 viewport에 추가해야 한다.
...
이제 화면 전체 영역을 사용할 수 있게 되었다. 이제 상단 막대 영역만큼 밀어야 할 필요가 생긴다.
